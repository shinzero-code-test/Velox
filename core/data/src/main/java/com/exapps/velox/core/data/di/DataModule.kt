...
import com.exapps.velox.core.data.preferences.EqualizerPreferencesStoreAdapter
import com.exapps.velox.core.data.preferences.ThemeRegistryAdapter
import com.exapps.velox.core.data.preferences.UserSettingsPreferencesStoreAdapter
...
internal val ALL_DATABASE_MIGRATIONS: Array<Migration> = arrayOf(
    ...
    // Phase 3 / Wave 3 / Round 2 — audio analysis (silence
    // detection + auto chapter generation). Two new tables
    // with foreign keys to media_items, both with CASCADE
    // delete so removing a track cleans up its analysis rows.
    object : Migration(3, 4) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """CREATE TABLE IF NOT EXISTS `track_intro_outro` (
                   `mediaItemId` INTEGER NOT NULL,
                   `kind` INTEGER NOT NULL,
                   `startMs` INTEGER NOT NULL,
                   `endMs` INTEGER NOT NULL,
                   PRIMARY KEY(`mediaItemId`, `kind`),
                   FOREIGN KEY(`mediaItemId`) REFERENCES `media_items`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE)
                """.trimIndent()
            )
            db.execSQL("CREATE INDEX IF NOT EXISTS index_track_intro_outro_mediaItemId ON track_intro_outro (mediaItemId)")
            db.execSQL(
                """CREATE TABLE IF NOT EXISTS `track_chapters` (
                   ...
                   PRIMARY KEY(`mediaItemId`, `index`),
                   FOREIGN KEY(`mediaItemId`) REFERENCES `media_items`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE)
                """.trimIndent()
            )
            db.execSQL("CREATE INDEX IF NOT EXISTS index_track_chapters_mediaItemId ON track_chapters (mediaItemId)")
        }
    },
)
...
object DatabaseModule {
    ...
    @Provides
    fun provideTrackAnalysisDao(database: VeloxDatabase): com.exapps.velox.core.data.local.dao.TrackAnalysisDao =
        database.trackAnalysisDao()
...
abstract class RepositoryModule {
    ...
    // Phase 3 / L6 (deferred-backlog): the player engine depends on these
    // domain ports only. The adapters live in :core:data and own the
    // DataStore shape; the engine never imports :core:data directly.
    @Binds
    @Singleton
    abstract fun bindDecoderPreferenceStore(
        impl: UserSettingsPreferencesStoreAdapter,
    ): DecoderPreferenceStore

    @Binds
    @Singleton
    abstract fun bindEqualizerPreferencesStore(
        impl: EqualizerPreferencesStoreAdapter,
    ): EqualizerPreferencesStore

    // Phase 3 / Milestone 2 — Theme engine. The registry implementation
    // is a Singleton here; the active-theme StateFlow is hot via
    // `stateIn(Eagerly)` so the UI can read it without subscribing.
    @Binds
    @Singleton
    abstract fun bindThemeRegistry(impl: ThemeRegistryAdapter): ThemeRegistry

    // Phase 3 / Wave 3 / Round 3 — Milestone 7: on-device
    // recommendation engine. Binds the port to the
    // `:core:data`-hosted implementation that reads play history
    // + the media_items table directly.
    @Binds
    @Singleton
    abstract fun bindRecommendationEngine(
        impl: com.exapps.velox.core.data.recommendation.RecommendationEngineImpl,
    ): com.exapps.velox.core.domain.recommendation.RecommendationEngine

    // Phase 3 / Wave 3 / Round 3.5d — APK-form plugin discovery.
    // The v1.8.0 host returns an empty list (no third-party
    // consumers on the horizon); the interface lives in
    // `:core:domain` so a real implementation can drop in.
    //
    // Phase 3 / Wave 3 / Round 3.5e — replaced by a real
    // PackageManager walker. The host's manifest declares the
    // signature permission and the `<queries>` element; this
    // class honours both. See
    // [com.exapps.velox.core.data.plugin.PackageManagerPluginDiscovery]
    // for the full contract.
    @Binds
    @Singleton
    abstract fun bindPluginDiscovery(
        impl: com.exapps.velox.core.data.plugin.PackageManagerPluginDiscovery,
    ): com.exapps.velox.core.domain.plugin.PluginDiscovery
}
